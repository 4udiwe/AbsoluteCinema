...
fun Movie.getRating(): Double? {
    return if (rating?.kp != 0.0)
        rating!!.kp
    else if (rating?.imdb != 0.0)
        rating!!.imdb
    else if (rating?.tmdb != 0.0)
        rating!!.tmdb
    else if (rating?.filmCritics != 0.0)
        rating!!.filmCritics
    else if (rating?.russianFilmCritics != 0.0)
        rating!!.russianFilmCritics
    else if (rating?.await != 0.0)
        rating!!.await
    else
        null
}